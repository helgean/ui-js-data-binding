...
export class UiJsDataBind {
    ...
    bind(el, propName) {
        ...
        this.populateBindedControl(prop, this.plainData[prop]);
    }

    populateBindedControl(prop, value) {
        // Update binded element with new value
        for (let elRef of this.bindings[prop]) {
            const el = elRef.deref();
            const dataType = (el.getAttribute('data-type') || "text").toLowerCase();
    ...
    observe(obj) {
        this._data = new Proxy(obj, {
            set: (target, key, value) => {
                ...
                // Update binded element with new value
                this.populateBindedControl(key, newValue);
